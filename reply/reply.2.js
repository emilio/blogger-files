/*
 * Script experimental para poder poner un doble botón
 * Testado, pero requiere más cambios en el DOM
 */
(function(window, document, undefined){
	...
	var comments = $$('comments'),
		...
		currentLink = null,
		currentRelatedLink = null;


	// Configurar los atributos necesarios para cancelar la respuesta
	// Set the reply attributes
	function setReplyState(link, type){
		if(link){
			link.setAttribute("data-reply-state", type);
			link.innerHTML = type === "none" ? REPLY_TEXT : CANCEL_TEXT;
		}
	}
	...
	function replyOriginal(){
		...
		setReplyState(currentLink, "none");
		setReplyState(currentRelatedLink, "none");
		currentLink = currentRelatedLink = null;
	...
	addEvent(replyLinks, 'click', function(ev){
		...
		var commentId = this.getAttribute("data-comment-id"),
			...
			// El otro botón de responder
			relatedReplyLink = $$((/^first/.test(this.id) ? "second-reply-link-" : "first-reply-link-") + commentId);
		...
		if( this.getAttribute("data-reply-state") === "replying" ){
			...
			setReplyState(this, "none");
			setReplyState(relatedReplyLink, "none");
		...
		// Primero cancelamos cualquier respuesta abierta
		// First we cancel any reply
		setReplyState(currentLink, "none");
		setReplyState(currentRelatedLink, "none");
